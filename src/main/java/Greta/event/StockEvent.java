...
public class StockEvent extends Event {
	...
	public StockEvent (int i, String t, int d, double o, double h, double l, double c, int v) {
		super(i, d);
		company = t;
		price = h;
		type = v / 10000;
	...
	public static StockEvent parse (int id, String line) {
		
		String[] values = line.split(",");
		
		String t = values[0]; 
		int d = Integer.parseInt(values[1].substring(6));
        double o = Double.parseDouble(values[2]); 
        double h = Double.parseDouble(values[3]); 
        double l = Double.parseDouble(values[4]);
        double c = Double.parseDouble(values[5]); 
        int v = Integer.parseInt(values[6]);
    	    	    	
    	StockEvent event = new StockEvent(id,t,d,o,h,l,c,v);    	
//    	System.out.println(event.toStringComplete());
	...
	public String toStringComplete() {
		return "id " + id + " sec " + sec + " type " + type + " company " + company + " price " + price;
	}
