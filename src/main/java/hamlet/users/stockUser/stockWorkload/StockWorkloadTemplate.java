...
@Data
public class StockWorkloadTemplate {

    //a list of queries
    private ArrayList<stockQuery> queries = new ArrayList<>();
     ...
     */
    public StockWorkloadTemplate(int queryNum, int miniWorkloads){
        ...
        for (int i =0; i<miniWorkloads; i++){
            ...
            for (int j=0;j<queriesPerGroup;j++){
               stockQuery query = new stockQuery(sharedET, predicate, aggreg);
               queries.add(query);
            }
        }
     ...
     * @param queries
     */
    public StockWorkloadTemplate(ArrayList<stockQuery> queries){
        this.queries = queries;

    }

    public void addQuery(stockQuery q){
        this.queries.add(q);
