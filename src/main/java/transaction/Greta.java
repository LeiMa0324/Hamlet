...
//	boolean MQ = false;
	...
	public Greta (Stream str, SingleQueryTemplate t, CountDownLatch d, AtomicLong time, AtomicInteger mem, boolean compr) {
		super(str,d,time,mem);
		query = t;
		compressed = compr;
	}
	public Greta (Stream str, Template t, CountDownLatch d, AtomicLong time, AtomicInteger mem, boolean compr) {
		super(str,d,time,mem);

	}

	public Greta(Stream stream, Template t, CountDownLatch done, AtomicLong latency, AtomicInteger memory) {
		super(stream,done,latency,memory);
	}
...
//			if (MQ) {
//				graph = graph.getGraph(events, template);
//			} else {
//				if (compressed) {
